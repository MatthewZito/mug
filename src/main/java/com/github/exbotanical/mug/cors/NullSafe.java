...
final class NullSafe {
   ...
   * @return The first value, or null if not extant.
   */
  static <T> T getFirst(List<T> listable) {
   ...
   * @return The first value, or null if not extant.
   */
  static <K, V> V getFirst(Map<K, List<V>> mappable, K key) {
   ...
   */
  static <T> T getFirstOrDefault(Iterable<T> iterable, T defaultValue) {
    if (iterable == null) {
      return defaultValue;
    }
